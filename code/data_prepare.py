...
parser.add_argument('--train', type=str, help='Size of the trainset (e.g., 0.5k)', default="0.5")
parser.add_argument('--train_type', type=str, help='LS/S of the trainset', default="LS")
parser.add_argument('--test', type=str, help='Size of the testset (e.g., 0.5k)', default="50")
parser.add_argument('--test_type', type=str, help='LS/S of the testset (e.g., LS)', default="LS")
...
parser.add_argument('--temp', type=bool, help='Yes or No use Temp Logging', default=False)
parser.add_argument('--new_pyg', type=int, help='if TestSet uses new pyg for embedding', default=0)
...
npy_file = f"own_data/{args.train_type}_Hm_{args.train}k_seed{data_seed}_11.npy"
if args.test_type == "S":
  LS_npy_file = f"own_data/S_Hm_{args.test}k_seed7_11.npy"
elif args.test_type == "LS":
  # LS_npy_file = f"own_data/LS_Hm_{args.test}k_v{(int(in_device)+1)*7}_11.npy"
  LS_npy_file = f"own_data/LS_Hm_{args.test}k_seed7_11.npy"
else:
  LS_npy_file = "own_data/" +args.test_type
    ...
    def __getitem__(self, index):
      try:
        try:
          ...
          # print(f'fetched prot: {prot1,prot2}')
    ...
    def __getitem__(self, index):
      try:
        try:
          ...
          # print('\n###')
          # print(f'Trying to fetch Pair:  <{self.protein_1[index], self.protein_2[index]}>')
          # print('###')
          ...
          #print(f'Here lies {glob.glob(prot_2)}')
          prot_2 = torch.load(glob.glob(prot_2)[0])
          # print(f'>Successfully fetched Pair, Data is: {prot_1,prot_2}')
          ...
          # print('\n### Encountered Regular Name')
          # print(f'>Trying to fetch Pair:  <{self.protein_1[index], self.protein_2[index]}>')
          # print(f' at {prot_1},{prot_2}')
          # print('###')

          prot_1 = torch.load(glob.glob(prot_1)[0])
          prot_2 = torch.load(glob.glob(prot_2)[0])
          if not args.new_pyg >= 1:
            prot_1 = bump(prot_1)
          if not args.new_pyg >= 2:
            prot_2 = bump(prot_2)   
          # print(f'>Successfully fetched Pair, Data is: {prot_1,prot_2}')
          return prot_1, prot_2, torch.tensor(self.label[index])
...
# Save LS_npy_data to pred_out if Predicting
if args.new_pyg >= 1:
  np.save(f"pred_out/{LS_npy_file.split('/')[-1]}", LS_npy_data)
  print(f">>> Saved Predicted Array to <pred_out/{LS_npy_file}>")
   

LS_testset = LabelledDataset_test(npy_file = LS_npy_data ,processed_dir= processed_dir)
LS_testloader = DataLoader_n(dataset= LS_testset, batch_size= 1, num_workers = 0, shuffle=False)
...
for i, row in enumerate(LS_npy_data):
    ...
    # print(prot1, prot2, prot_count_in_seen)
...
print(f'> Shape of TrainSet: {npy_data.shape}')
print(f'> Shape of TestSet: {LS_npy_data.shape}')
...
# # Initialize counters
# rows_with_false = 0
# total_false_protein_ids = 0

# # Directory pattern
# directory_pattern = "own_data/processed/*{}*"

# # Iterate through each row
# pbar = tqdm(npy_data)
# for row in pbar:
#     # Extract protein IDs from columns 3 and 6 (0-based indexing: 2 and 5)
#     protein_id1 = row[2]
#     protein_id2 = row[5]
    
#     # Check existence for protein_id1
#     pattern1 = directory_pattern.format(protein_id1)
#     exists1 = bool(glob.glob(pattern1))
    
#     # Check existence for protein_id2
#     pattern2 = directory_pattern.format(protein_id2)
#     exists2 = bool(glob.glob(pattern2))
    
#     # Determine if any protein ID does not have a matching file
#     row_has_false = False
#     if not exists1:
#         total_false_protein_ids += 1
#         row_has_false = True
#     if not exists2:
#         total_false_protein_ids += 1
#         row_has_false = True
...
#     if row_has_false:
#         rows_with_false += 1
#     pbar.set_description(f"Rows has/with False: {total_false_protein_ids,rows_with_false}")

# # Print the results
# print(f"Number of lines with at least one False: {rows_with_false}")
# print(f"Total number of False protein_ids: {total_false_protein_ids}")